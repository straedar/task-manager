...
export type MapSettings = {
  ...
  hasFloorTexture: boolean;
  floorUrl: string | null;
};

export function fetchMapSettings() {
  return requestJson<MapSettings>("/map-settings");
}

export function updateMapSettings(patch: Partial<MapSettings>) {
  return requestJson<MapSettings>("/map-settings", {
    method: "PUT",
    body: JSON.stringify(patch),
  });
...
export function uploadFloorTexture(mime: string, dataBase64: string) {
  return requestJson<{ ok: boolean; floorUrl: string }>("/map-settings/floor", {
    method: "POST",
    body: JSON.stringify({ mime, dataBase64 }),
  });
}

export function clearFloorTexture() {
  return requestJson<{ ok: boolean }>("/map-settings/floor", {
    method: "DELETE",
  });
}